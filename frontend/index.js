function moduleProject3() {
  ...
  function buildLearnerCard(learner, languages) {
    ...
    const favorLangP = document.createElement('p');
    const favLanguage = languages.find(lang => lang.id === learner.favLanguage)
    favorLangP.textContent = `Favorite Language: ${favLanguage.name}`;

    [nameP, dobP, favorLangP, idElement].forEach(p => {
      card.appendChild(p)
    })
  ...
  function buildFooter(footerData) {
    ...
    [companyNameP, addressP, contactEmailP].forEach(p => {
    ...
    for(let social in footerData.socialMedia) {
      ...
      socialMediaLink.textContent = social.charAt(0).toUpperCase() + social.slice(1);
